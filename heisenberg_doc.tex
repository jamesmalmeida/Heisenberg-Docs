...
\newtcolorbox{dica}{
  ...
  title={\faLightbulb\quad Dica},
  fonttitle=\bfseries, breakable
}
\newtcolorbox{atencao}{
  colback=warnyellow, colframe=warnborder,
  title={\faExclamationTriangle\quad Atenção},
  fonttitle=\bfseries, breakable
}
\newtcolorbox{info}{
  colback=infoblue, colframe=infoborder,
  title={\faInfoCircle\quad Informação},
  fonttitle=\bfseries, breakable
}
\newtcolorbox{perigo}{
  colback=dangerred, colframe=dangerborder,
  title={\faSkull\quad Cuidado},
  fonttitle=\bfseries, breakable
}
...
\begin{titlepage}
  ...
  % >>> IMAGEM 1: Insira aqui o logo do CNPEM/Ilum <<<
  % \includegraphics[width=0.35\textwidth]{logo_cnpem.png}\\[1.5cm]
  \textbf{\Large [Inserir logo do CNPEM / Ilum aqui]}\\[1.5cm]
  ...
  \begin{tcolorbox}[colback=infoblue, colframe=infoborder, width=0.75\textwidth]
    ...
    \textbf{Reset de senha:} \texttt{http://172.20.60.13}
  \end{tcolorbox}
...
% >>> IMAGEM 2: Diagrama de arquitetura do cluster (nó de login + nós de trabalho) <<<
% Sugestão: diagrama mostrando o usuário → nó de login (heisenberg.cnpem.br) →
%           rede interna → nós work0..work8
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.85\textwidth}{\centering\vspace{2cm}
    \textbf{[Inserir diagrama: Usuário $\rightarrow$ Nó de Login $\rightarrow$
    Nós de Trabalho work0--work8]}
    \vspace{2cm}}}
...
Substitua \texttt{seu\_usuario} pelo seu login institucional.
...
% >>> IMAGEM 3: Screenshot do PowerShell conectando ao cluster via SSH <<<
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.85\textwidth}{\centering\vspace{2.5cm}
    \textbf{[Inserir screenshot: PowerShell executando o comando ssh
    e exibindo o prompt do cluster após o login]}
    \vspace{2.5cm}}}
...
% >>> IMAGEM 4: Screenshot do terminal logo após o login no Heisenberg <<<
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.85\textwidth}{\centering\vspace{2.5cm}
    \textbf{[Inserir screenshot: terminal após login, mostrando o prompt
    ``usuario@heisenberg:\textasciitilde\$'']}
    \vspace{2.5cm}}}
  \caption{Prompt do terminal após o login no Heisenberg.}
\end{figure}
...
% >>> IMAGEM 5: Screenshot do editor nano com um arquivo aberto <<<
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.85\textwidth}{\centering\vspace{2.5cm}
    \textbf{[Inserir screenshot: editor nano com arquivo aberto e barra
    de atalhos na parte inferior]}
    \vspace{2.5cm}}}
...
Atalhos do \cmd{nano}:
\begin{itemize}
  \item \texttt{Ctrl+O} → salvar (\textit{O de Output})
  \item \texttt{Ctrl+X} → sair
  \item \texttt{Ctrl+W} → buscar texto
\end{itemize}
...
Substitua \texttt{james} pelo seu usuário.
...
% >>> IMAGEM: Screenshot do WinSCP conectado ao Heisenberg <<<
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.85\textwidth}{\centering\vspace{2cm}
    \textbf{[Inserir screenshot: WinSCP com conexão ao Heisenberg aberta,
    mostrando arquivos locais à esquerda e arquivos do cluster à direita]}
    \vspace{2cm}}}
...
\subsection{Executar Programas em Segundo Plano}

Normalmente, quando você executa um comando no terminal, ele ocupa o prompt
até terminar. Para liberar o terminal enquanto o programa ainda roda, use o
\textbf{\&} ao final do comando:
...
O número entre colchetes é o número do job e o seguinte é o PID do processo.
O programa continua rodando em segundo plano enquanto você usa o terminal
normalmente.

O problema do \textbf{\&} sozinho é que, se você fechar a sessão SSH, o
programa é encerrado junto. Para evitar isso, use \cmd{nohup}
(\textit{no hang up}), que desconecta o processo da sessão:
...
O \texttt{> saida.out} redireciona a saída padrão e \texttt{2> erros.err}
redireciona os erros, pois com \cmd{nohup} o terminal não está mais disponível
para exibir nada. O programa continuará rodando mesmo após você fechar o SSH.
...
\begin{atencao}
  O \cmd{nohup} serve para tarefas leves e rápidas, como scripts de pré ou
  pós-processamento. \textbf{Não rode simulações pesadas diretamente no nó
  de login}, independentemente de usar \cmd{nohup} ou não. Para isso, utilize
  sempre o Slurm.
\end{atencao}
...
\begin{itemize}
  \item \textbf{Do you accept the license terms?} → Digite \texttt{yes} e pressione
        \texttt{Enter}.
  \item \textbf{Confirm install location} → Pressione \texttt{Enter} para aceitar
        o padrão (\texttt{\textasciitilde/miniconda3}).
  \item \textbf{Do you wish the installer to initialize Miniconda3?} → Digite
...
\begin{longtable}{ll}
  ...
  \texttt{jupyter} & Notebooks interativos (Jupyter Notebook / Lab) \\
  ...
  \texttt{MDAnalysis} & Análise de trajetórias de dinâmica molecular \\
  \texttt{ase} & Atomic Simulation Environment (materiais e moléculas) \\
  \bottomrule
...
\begin{tcolorbox}[colback=infoblue, colframe=infoborder,
                  title={\faHeadset\quad Onde buscar ajuda}, fonttitle=\bfseries]
\begin{description}
  \item[\faTicketAlt\ Sistema de chamados:] \url{https://web-ilum.cnpem.br/chamado}\\
  ...
  \item[\faKey\ Reset de senha:] \url{http://172.20.60.13}\\
    Acesse dentro da rede Ilum ou via VPN para redefinir sua senha.
  \item[\faServer\ Endereço do cluster:] \texttt{heisenberg.cnpem.br}
\end{description}
\end{tcolorbox}
